...
public class AppSchedulesMealRecipeController {
    ...
    public AppSchedulesMealRecipeController(DayRepository dayRepository, RecipeRepository recipeRepository, PlanRepository planRepository){
    ...
    public String getAppSchedulesMealRecipePage(Model model){
        ...
        model.addAttribute("scheduleRecipe", new Plan());
        return "app-schedules-meal-recipe";
    }

    @PostMapping("/app/schedulesmealrecipe/add")
    public String processAppScheduleMealRecipePage(@ModelAttribute Plan plan){
        Plan existPlan = planRepository.findByName(plan.getName());
        plan.setId(existPlan.getId());
        plan.setAdmin(existPlan.getAdmin());
        plan.setDescription(existPlan.getDescription());
        plan.setCreated(existPlan.getCreated());
